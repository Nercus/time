...
<template>
  <div v-if="!isArrow" class="flex flex-col items-center gap-2">
    ...
    <div class="gap-4 grid max-w-30" :class="columns">
      <Motion v-for="i in props.max" :key="i" :animate="props.num >= i ? { scale: 1 } : { scale: 0.3 }" :initial="{ scale: 0.3 }" :transition="{ type: 'spring', duration: 0.5, bounce: 0.3 }" class="flex justify-center items-center rounded-full size-3 font-mono text-lg select-none will-change-transform" :class="props.num >= i ? color : 'bg-gray-200'" />
    </div>
  </div>
